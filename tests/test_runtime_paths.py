...
def test_codex_usage_database_uses_app_data_root(monkeypatch, tmp_path: Path):
    data = tmp_path / "data"
    monkeypatch.setenv("TCB_DATA_DIR", str(data))

    path = runtime_paths.get_codex_usage_db_path()

    assert path == data / "codex-usage" / "usage.sqlite3"
