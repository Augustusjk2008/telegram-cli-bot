...
def test_codex_switch_source_script_starts_with_human_readable_title():
    script_path = Path("scripts/codex_switch_source.bat").resolve()
    first_line = script_path.read_text(encoding="utf-8").splitlines()[0].strip()

    assert first_line == ":: Codex 换源"
