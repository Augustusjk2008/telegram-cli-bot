...
def test_load_terminal_actions_config_reads_valid_actions(tmp_path: Path) -> None:
    _write_config(
        tmp_path,
        {
            "schemaVersion": 1,
...
def test_save_terminal_actions_config_writes_normalized_json(tmp_path: Path) -> None:
    saved = save_terminal_actions_config(
        tmp_path,
        {
            "schemaVersion": 1,
            "actions": [
                {"id": "test", "label": "测试", "icon": "TestTube2", "command": "python -m pytest tests -q"}
            ],
        },
        expected_mtime_ns="",
    )
...
def test_resolve_terminal_action_requires_confirmation(tmp_path: Path) -> None:
    _write_config(
        tmp_path,
        {
            "schemaVersion": 1,
            "actions": [
                {"id": "clean", "label": "清理", "icon": "Trash2", "command": "git clean -fdx", "confirm": True}
            ],
        },
    )
