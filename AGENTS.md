...
- 可选 LiteLLM 网关提供 OpenAI 兼容转接、多路由和 endpoint 模式切换；普通 CLI 默认仍可直接连接其自身 provider
...
- 网关配置：`bot/web/transfer_litellm_config.py`
- LiteLLM 子进程运行时：`bot/web/transfer_litellm_runtime.py`
- 请求转发、统计和 Codex compact 适配：`bot/web/transfer_service.py`
- 管理 API 和独立配置页：`bot/web/routes/transfer_routes.py`
- Admin Center 前端入口：`front/src/screens/AdminCenterScreen.tsx`

关键行为：

- LiteLLM 网关是可选能力，通过独立 `enabled` 状态启停；配置变化支持热切换，不要求重启主 Web 服务
- 支持多路由；每条路由包含模型别名、LiteLLM model、上游 provider 地址、密钥、`endpoint_mode` 和额外 LiteLLM 参数
- `endpoint_mode` 仅允许 `auto`、`chat_completions`、`responses`
- Codex remote compact 请求经 Responses 入口转发时，由网关把 LiteLLM 流式结果转换为 Codex 需要的 compact 事件格式
- 配置和日志默认写入 `get_app_data_root() / "transfer"` 下的 `litellm-config.yaml`、`litellm.log`；trace 仍由 `TRANSFER_TRACE=1` 控制
...
- LiteLLM 网关运行态配置和日志不在仓库根；从 `bot/runtime_paths.py:get_transfer_litellm_config_path()` / `get_transfer_litellm_log_path()` 获取，默认位于 `Path.home() / ".tcb" / "orbit-safe-claw" / "transfer"`
...
- LiteLLM 网关配置、运行时、路由和 Codex compact 转换测试：`tests/test_transfer_service.py`、`tests/test_transfer_routes.py`
