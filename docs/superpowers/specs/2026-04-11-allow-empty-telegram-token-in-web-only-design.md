...
允许用户在纯 Web 模式下将 `.env` 中的 `TELEGRAM_BOT_TOKEN` 留空，并保证系统在这种模式下不依赖 Telegram 轮询；若仍显式配置了有效 token 且 `ALLOWED_USER_IDS` 可用，Quick Tunnel 允许通过临时 bot 推送公网地址。
...
3. 纯 Web 模式下不启动 Telegram 轮询。
4. Cloudflare Quick Tunnel 在纯 Web 模式下仍可正常工作；当 token 留空时，公网地址只在 Web 页面和本机剪贴板中可见；当主 profile 仍配置有效 token 且 `ALLOWED_USER_IDS` 可用时，允许通过临时 bot 推送。

## 用户确认的约束

- 只有 `TELEGRAM_ENABLED=false` 时才允许空 token。
- 只要 Telegram 关闭，就不应启动 Telegram 轮询；若 token 留空，则不应有任何 Telegram 相关动作。
...
- [bot/web/server.py](C:/Users/JiangKai/telegram_cli_bridge/refactoring/bot/web/server.py) 的 tunnel 通知逻辑会尝试把公网地址复制到剪贴板；当主 Telegram application 存在时优先复用它发送消息，在纯 Web 模式下若主 profile 仍配置了有效 token 且 `ALLOWED_USER_IDS` 可用，则会临时构造 bot 发送通知。
...
- Tunnel 公网地址通知保持“复制到剪贴板”的本地行为；若主 Telegram application 已启动则复用它推送，若 Telegram 轮询关闭但主 profile 仍配置了有效 token 且 `ALLOWED_USER_IDS` 可用，则允许通过临时 bot 推送。
...
- 主 Telegram application 存在时，优先复用它做 Telegram 推送
- 主 Telegram application 不存在但主 profile 仍配置了有效 token 且 `ALLOWED_USER_IDS` 可用时，允许通过临时 bot 推送
- 若以上条件都不满足，直接返回“不发送 Telegram 通知”的结果，不视为错误
...
- 若 token 非空且 `ALLOWED_USER_IDS` 可用，则 tunnel 地址仍可能通过临时 bot 推送
...
7. 获得公网地址后：
   - 若 token 为空或 `ALLOWED_USER_IDS` 不可用，只做本地剪贴板复制，不发 Telegram 消息
   - 若 token 非空且 `ALLOWED_USER_IDS` 可用，则允许通过临时 bot 推送公网地址
...
- 纯 Web 模式下 tunnel 启动成功但没有 Telegram application：若 token 为空或 `ALLOWED_USER_IDS` 不可用，不记为失败，也不重试 Telegram 推送
...
- `_notify_tunnel_public_url()` 在 token 为空时只复制剪贴板、不发送消息
- Web server 自动启动 tunnel 时，在纯 Web 模式下若 token 非空且 `ALLOWED_USER_IDS` 可用，会通过临时 bot 推送公网地址
