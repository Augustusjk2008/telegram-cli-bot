...
- `busy_policy`
  - v1 默认 `wait`
  - 含义是 job 到点但 assistant 忙时，先排队等待
  - v1 可额外支持 `skip`，但不作为默认
- `session_mode`
  - `persistent`：长期复用同一个 synthetic user
  - `ephemeral`：每次运行结束后清理该 synthetic user session
...
调度器仍然建议使用 `croniter`，不必上 APScheduler。

主循环：

1. 启动时扫描 `.assistant/automation/jobs/*.yaml`
2. 计算每个 job 的 `next_run_at`
...
- `busy_policy=wait` 时，允许保留一个 pending run
...
- `ignore_if_late`
  - 若超出 `max_lag_seconds`，则不补跑

补跑进入队列后，仍然遵守统一串行和 `busy_policy`。
...
- cron 表达式解析和 `next_run_at` 计算
- Windows / Linux 下路径落盘一致性
- startup misfire 行为
- `busy_policy=wait` 的排队行为
- cron 与人工消息的先后顺序
- 同一 job 重复触发时的 `coalesced_count`
- synthetic `user_id` 稳定性
- `persistent` 和 `ephemeral` session 行为
...
最大风险仍然不是 cron 解析，而是 assistant 运行模型。
