...
def _build_updated_frontend(repo_root: Path) -> tuple[bool, str]:
    ...
    # scripts/build_web_frontend.* 已随 1.5.0 脚本清理移除；缺失时与
    # start.sh 保持一致：直接 npm run build，失败后安装依赖重试一次。
    npm_path = shutil.which("npm")
    if not npm_path:
        return False, "未找到前端构建脚本，且系统中没有 npm，无法重建前端资源"

    front_root = repo_root / "front"
    build_success, build_output = _run_frontend_build_command([npm_path, "run", "build"], front_root)
    if build_success:
        return build_success, build_output

    install_success, install_output = _run_frontend_build_command([npm_path, "install"], front_root)
    if not install_success:
        return install_success, install_output
    return _run_frontend_build_command([npm_path, "run", "build"], front_root)
