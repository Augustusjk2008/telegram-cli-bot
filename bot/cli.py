...
def _extract_kimi_content_text(content: Any) -> Optional[str]:
    if isinstance(content, str) and content.strip():
        return content.strip()
    if not isinstance(content, list):
        return None
...
def _extract_kimi_error_text(event: Dict[str, Any]) -> Optional[str]:
    for key in ("message", "error", "detail"):
        value = event.get(key)
        if isinstance(value, str) and value.strip():
            return value.strip()
        if isinstance(value, dict):
            for nested_key in ("message", "detail", "error"):
                nested = _extract_nested_nonempty_str(value, nested_key)
                if nested:
                    return nested
    return None


def parse_kimi_stream_json_line(line: str) -> Dict[str, Optional[str]]:
    """解析 Kimi --output-format stream-json 单行事件。"""
    result: Dict[str, Optional[str]] = {
        "completed_text": None,
        "delta_text": None,
        "error_text": None,
    ...
    if event_type == "error":
        result["error_text"] = _extract_kimi_error_text(event)
        return result

    error_text = _extract_kimi_error_text(event)
    role = str(event.get("role") or "").strip().lower()
    if error_text and (event_type.lower() == "error" or not role):
    ...
    text_value = _extract_kimi_content_text(event.get("content"))
    if text_value:
        result["completed_text"] = text_value
        result["delta_text"] = text_value
    return result
...
def parse_kimi_stream_json_output(raw_output: str) -> str:
    """解析 Kimi stream-json 完整输出，返回最后一段 assistant 文本。"""
    raw_lines: List[str] = []
    completed_parts: List[str] = []
    delta_parts: List[str] = []
    error_parts: List[str] = []
    ...
    if completed_parts:
        ...
        final_text = "\n".join(part for part in delta_parts if part).strip()
    if not final_text and error_parts:
...
def extract_kimi_error_output(raw_output: str) -> Optional[str]:
    """提取 Kimi JSON 输出里的 error 文本。"""
    error_parts: List[str] = []
    for line in raw_output.splitlines():
        stripped = line.strip()
        if not stripped:
