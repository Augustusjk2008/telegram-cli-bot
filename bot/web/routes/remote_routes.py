...
def _relative_target(path: Any, current: str) -> str:
    text = str(path or "").strip()
    return posixpath.join(current, text) if text else current
...
async def create_terminal(server, request: web.Request, auth, body: dict[str, Any], size) -> web.Response | None:
    ...
    cwd = await asyncio.to_thread(resolve_remote_directory, connection, cwd, config["root"])
...
async def _workspace_request(server, request, profile, suffix, browse_dirs):
    ...
    if suffix == "ls":
        return _response(await asyncio.to_thread(
            connection.list_directory, _relative_target(request.query.get("path"), current), root=root,
        ))
    if suffix == "files/read":
        ...
        data = await asyncio.to_thread(
            connection.read_file, _relative_target(request.query.get("filename"), current), root=root, limit=limit,
        )
        return _response(data)
    body = await server._parse_json(request)
    if suffix == "cd":
        directory = await asyncio.to_thread(resolve_remote_directory, connection, _relative_target(body.get("path"), current), root)
        browse_dirs[key] = directory
        return _response({"working_dir": directory, "is_virtual_root": False})
    if suffix == "files/reveal":
        return _response(await asyncio.to_thread(_reveal, connection, root, current, body.get("path")))
    if suffix == "files/write":
        if not isinstance(body.get("content"), str):
            raise WebApiError(400, "invalid_content", "文件内容必须是文本")
        return _response(await asyncio.to_thread(
            connection.write_file, _relative_target(body.get("path"), current), body["content"], root=root,
    ...
    if suffix in {"files/create", "files/mkdir"}:
        ...
        parent = _relative_target(body.get("parent_path"), current)
...
def _reveal(connection, root: str, current: str, path: Any) -> dict[str, Any]:
    target = posixpath.normpath(_relative_target(path, current))
...
def register(app: web.Application, server) -> None:
    ...
    async def directories(request):
        ...
        connection = await asyncio.to_thread(service.get, config)
        result = await asyncio.to_thread(connection.list_directory, request.query.get("path") or config["root"], root="/")
        return _response(result)
    ...
    async def reconnect(request):
        ...
        config = {**profile.remote_workspace}
        for field in ("key_filename", "host_key_fingerprint"):
            if field in body and body[field] != config.get(field, ""):
