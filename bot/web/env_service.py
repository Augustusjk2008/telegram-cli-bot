...
ENV_SCHEMA: tuple[EnvField, ...] = (
    ...
    EnvField("WEB_LOGIN_MAX_ATTEMPTS", "登录失败阈值", "同一客户端与账号在观察窗口内允许的失败次数。", "number", "5", "web", min_value=2, max_value=100, integer=True),
    EnvField("WEB_LOGIN_WINDOW_SECONDS", "登录观察窗口", "统计连续登录失败的秒数。", "number", "300", "web", min_value=10, max_value=86400, integer=True),
    EnvField("WEB_LOGIN_BASE_LOCK_SECONDS", "登录基础冷却", "达到阈值后的初始冷却秒数，后续失败指数增加。", "number", "2", "web", min_value=1, max_value=3600, integer=True),
    EnvField("WEB_LOGIN_MAX_LOCK_SECONDS", "登录最大冷却", "单次登录冷却的最大秒数。", "number", "60", "web", min_value=1, max_value=86400, integer=True),
    EnvField("WEB_TRUST_PROXY_HEADERS", "信任代理客户端 IP", "仅当直连来源为本机代理时读取 X-Forwarded-For。", "boolean", "false", "web"),
    ...
    EnvField("TCB_LSP_ENABLED", "启用语言服务", "启用语言服务器发现与后续代码导航；默认关闭，且不会自动下载工具。", "boolean", "false", "language-server"),
    EnvField("TCB_LSP_PYRIGHT_COMMAND", "Pyright 命令", "优先使用的 Pyright 语言服务器命令；留空后才检查 PATH 与托管版本。", "path", "", "language-server"),
    EnvField("TCB_LSP_TYPESCRIPT_COMMAND", "TypeScript 命令", "优先使用的 TypeScript Language Server 命令；留空后才检查 PATH 与托管版本。", "path", "", "language-server"),
    EnvField("TCB_LSP_CLANGD_COMMAND", "clangd 命令", "优先使用的 clangd 命令；留空后才检查 PATH 与托管版本。", "path", "", "language-server"),
    EnvField("TCB_LSP_REQUEST_TIMEOUT_SECONDS", "语言服务请求超时", "单次语言服务请求的超时秒数。", "number", "10", "language-server", min_value=1),
    EnvField("TCB_LSP_IDLE_TIMEOUT_SECONDS", "语言服务空闲回收", "语言服务空闲回收秒数。", "number", "300", "language-server", min_value=30),
    EnvField("TCB_LSP_MAX_RUNTIMES", "语言服务实例上限", "可同时保留的语言服务运行时数量上限。", "number", "8", "language-server", min_value=1, max_value=128, integer=True),
    EnvField("TCB_LSP_EXTERNAL_SOURCES_ENABLED", "允许外部依赖源码", "允许后续语言服务导航读取经认可的外部依赖源码；默认关闭。", "boolean", "false", "language-server"),
