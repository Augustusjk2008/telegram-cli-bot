...
describe("RealWebBotClient", () => {
  ...
  test("transfer bridge status maps snake case response", async () => {
    fetchMock.mockResolvedValue(jsonOk({
      ...
      drop_params: true,
      litellm_proxy_base_url: "http://127.0.0.1:49152/v1",
      litellm_log_tail: ["ready"],
      request_count: 2,
  ...
  test("transfer admin endpoints use admin paths and serialize config", async () => {
    ...
    expect(status.providerBaseUrl).toBe("https://api.example.test/v1");
    expect(saved.litellmModel).toBe("openai/gpt-next");
  ...
  test("sendMessage keeps embedded done content when stream output disagrees", async () => {
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
      start(controller) {
        controller.enqueue(encoder.encode(`event: done\ndata: ${JSON.stringify({
          ...
          elapsed_seconds: 3,
          message: {
            id: "msg-final-authoritative-content",
            role: "assistant",
            content: "最终答复",
            state: "done",
