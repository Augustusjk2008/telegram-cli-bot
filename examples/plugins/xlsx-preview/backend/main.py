...
from xlsx_parser import parse_xlsx_document
...
def read_xlsx_bytes(path: str) -> bytes:
    response = call_host("host.workspace.read_bytes", {"path": path})
    if response.get("error"):
        raise RuntimeError(str(response["error"].get("message") or "读取 XLSX 失败"))
...
def render_view(input_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not path:
        raise RuntimeError("缺少 XLSX 路径")
    payload = parse_xlsx_document(path, read_xlsx_bytes(path))
    return {
        "renderer": "document",
...
for raw_line in sys.stdin:
    ...
    try:
        if method == "plugin.initialize":
            respond(request_id, {"ok": True, "name": "xlsx-preview"})
