...
def _xml(archive: ZipFile, name: str) -> ET.Element | None:
    try:
        ...
        if entry.file_size > 4 * 1024 * 1024:
            raise RuntimeError("PPTX XML 超过预览限额")
...
def _image_payload(
    ...
    if write_artifact is None:
        return None
    try:
        entry = archive.getinfo(target)
        if entry.file_size > 32 * 1024 * 1024:
            raise RuntimeError("PPTX 图片超过 32 MiB 预览限额")
...
def parse_pptx_document(
    ...
    *,
    ...
    try:
        with ZipFile(BytesIO(content)) as archive:
            if sum(entry.file_size for entry in archive.infolist()) > 256 * 1024 * 1024:
                raise RuntimeError("PPTX 解压后超过 256 MiB 预览限额，请压缩图片或拆分文稿")
            ...
            for slide_number, slide_target in enumerate(slide_targets[:max_slides], start=1):
                ...
                if slide_root is None:
                    raise RuntimeError(f"PPTX 缺少第 {slide_number} 页内容")
    ...
    return {
        ...
        "statsText": f"{slide_count} 页 · 基础静态预览（复杂图形和版式可能不完整）",
        "blocks": blocks,
    }
