...
def read_pdf_bytes(path: str) -> bytes:
    ...
    if response.get("error"):
        raise RuntimeError(str(response["error"].get("message") or "读取 PDF 失败"))
...
def write_artifact(filename: str, content: bytes, content_type: str) -> dict[str, Any]:
    response = call_host("host.temp.write_artifact", {
        "filename": filename,
        "contentBase64": base64.b64encode(content).decode("ascii"),
        "contentType": content_type,
    })
    if response.get("error"):
        raise RuntimeError(str(response["error"].get("message") or "写入 PDF 预览失败"))
...
def render_view(input_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not path:
        raise RuntimeError("缺少 PDF 路径")
    payload = parse_pdf_document(path, read_pdf_bytes(path), write_artifact=write_artifact)
    return {
        "renderer": "document",
...
for raw_line in sys.stdin:
    ...
    try:
        if method == "plugin.initialize":
            respond(request_id, {"ok": True, "name": "pdf-preview"})
